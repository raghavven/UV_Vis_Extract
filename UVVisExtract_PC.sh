...
cut -d'|' -f 1-10 1Ex.txt > 2Ex.txt | 	#Extracts columns (from 1 to 10)such as File Name, Exicted State, Singlet/Triplet, transition energies in eV and wavelength (nm), oscillator strength (OS) and spin and saves it as 2Ex.txt
sed -e "s/f=//g" -i.backup 2Ex.txt | 	#Deletes the string f= from the OS column on the file 2Ex.txt after taking its back up as 2Ex.txt.backup, this enables the next arithmetic command to execute successfully
